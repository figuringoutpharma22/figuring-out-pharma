...
def replace_block(html, start_marker, end_marker, new_content):
    ...
    if not pattern.search(html):
        raise RuntimeError(
            f"\nMarker not found in index.html:\n  {start_marker}\n"
            "Has the HTML structure changed? Check that both START and END "
            "markers are present and spelled exactly as in this script."
        )
...
def main():
    ...
    print()
    if len(beyond) == 0:
        print(f"Featured side cards: showing 2 fallback ARTICLES "
              f"(you have {len(case_studies)}/6 case studies in the grid)")
        print(f"  — Add case study #7 to trigger auto-rotation")
    elif len(beyond) == 1:
        print(f"Featured side cards: side 1 = case study #{len(case_studies)} "
              f"| side 2 = fallback article")
    else:
        print(f"Featured side cards: both auto-rotated from case studies "
              f"#{7} and #{8} (beyond grid)")
